...
# 可以使用多个模式。模式中一般需要包含模式字#符“%”。存在多个模式时，模式表达式之间使用空格分割。 
# 返回值为空格分割的“TEXT”字串中所有符合模式“PATTERN”的字串。
# “filter”函数可以用来去除一个变量中的某些字符串。

# $(basename NAMES…) 
# 取前缀函数—basename，从文件名序列“NAMES…”中取出各个文件名的前缀部分（点号之前的#部分）。
